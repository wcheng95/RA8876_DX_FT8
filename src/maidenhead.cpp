...
const char *get_mh(double lat, double lon, int size)
{
    ...
    static char locator_str[10]; // Max 10 chars for locator + null terminator
    int i;
    lon += LON_OFFSET;
    lat += LAT_OFFSET;
...
{
    if (strlen(locator) >= 10)
    {
        return locator; // Already complete or too long
    }
    
    static char completed_locator[10];
    memcpy(completed_locator, locator, sizeof(completed_locator));
    // Fill with default values for a 10-character locator
    // This assumes a standard pattern for completion
    const char default_fill[] = "LL55LL55LL";
    for (size_t i = strlen(locator); i < sizeof(completed_locator); ++i)
    {
        completed_locator[i] += default_fill[i];
    }
    return completed_locator;
}

double mh2lon(const char *locator_str)
{
    const char *locator = locator_str;
    if (strlen(locator) > 10)
        return 0.0;
    if (strlen(locator) < 10)
...
double mh2lat(const char *locator_str)
{
    const char *locator = locator_str;
    if (strlen(locator) > 10)
        return 0.0;
